    ...
    public static void performTreeCraft() {
        ...
        try {
            for (MaterialNode node : getTreeRecipes(BoM.tree.goal)) {
                ...
                if (EmiRecipeFiller.performFill(node.recipe, screen, EmiCraftContext.Type.FILL_BUTTON, EmiCraftContext.Destination.INVENTORY, (int) Math.ceil(node.neededBatches))) BoM.tree.recalculate();
                if (EmiAgnos.isModLoaded("toms_storage") && screenHandler instanceof CraftingTerminalMenu) for (int i = 0; i < Math.ceil(node.neededBatches); i++, client.interactionManager.clickSlot(screenHandler.syncId, 0, 0, SlotActionType.PICKUP, client.player));
                return;
            }
